...
class MMDLoader extends Loader {
  ...
  async loadAnimation(url: any, object: any, onProgress: Function, onError = () => {}) {
    ...
    return object.isCamera ? builder.buildCameraAnimation(vmd) : builder.build(vmd, object);
  }
  ...
  async loadWithAnimation(
    ...
    const { mesh } = await this.setModelParams(params).loadAsync(modelUrl, onProgress);
...
class MaterialBuilder {
    ...
    for (let i = 0; i < data.metadata.materialCount; i++) {
      ...
      if (!shaderParams.enablePBR) {
        ...
        if (material.envTextureIndex !== -1 && (material.envFlag === 1 || material.envFlag == 2)) {
          params.matcap = this._loadTexture(data.textures[material.envTextureIndex]);
      ...
      if (shaderParams.isWebGPU && shaderParams.enableSdef) {
        ...
        newMaterial = (
          shaderParams.enablePBR ? new MeshPhysicalMaterial(params) : new MMDToonMaterial(params)
        ) as MMDMaterial;
  ...
  _loadTexture(
    ...
    if (params.isDefaultToonTexture) {
      let index;
  ...
  _checkImageTransparency(
    ...
    const compressedMap = map as CompressedTexture;
    if (compressedMap.isCompressedTexture) {
      if (compressedMap.format in NON_ALPHA_CHANNEL_FORMATS) {
    ...
    const imageData = map.image.data !== undefined ? map.image : createImageData(map.image);
...
class AnimationBuilder {
  ...
  buildCameraAnimation(vmd: { cameras: any[] }) {
    ...
    const cameras = vmd.cameras?.slice() ?? [];
